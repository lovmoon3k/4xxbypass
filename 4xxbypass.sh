...
function print(){
  ...
  if [[ ${status} == "302" ]]; then
    ...
    location=$(echo ${payload} | grep -i "location:" | awk '{print $2}')
    printf "Redirecting to: ${location}\n"
    results+=("${location}")
  elif [[ ${status} =~ 2.. ]];then
  ...
  echo "Scan Results: ${results[@]}" | tee -a "scan_results.txt"
}

function banner(){
    echo "LET'S FIND SOME PAYLOADS AND BYPASS SOME 4XX" | lolcat
    echo -e        "${green} HACK THE PLANET ${end}"
    echo "Let me give you a sample to get you in the spirit of the game!" | lolcat
    echo -e "${cyan}- Another WishMaster Production- ${end}"
}

function usage(){
    printf "Usage:\n"
    printf "\t4xxbypass [URL]\n" >&2
    printf '\n' >&2
